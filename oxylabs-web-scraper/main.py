...
def render_product_card(product):
    with st.container(border=True):
        ...
        with cols[1]:
            ...
            info_cols[0].metric(label="Price", value=f"{currency}{price}" if currency else price)
...
def main():
    ...
    if products:
        ...
        st.write(f" Showing {start_index + 1}-{end_index} of {len(products)} products")
    ...
    if selected_asin:
        st.divider()
        st.subheader(f"Competitors analysis for {selected_asin}")

        db = Database()
        # First check if there are any existing competitors in the DB
        existing_competitors = db.search_products({"parent_asin": selected_asin})

        if not existing_competitors:
            with st.spinner("Searching..."):
                competitors = fetch_and_store_competitors(selected_asin, domain, geo)
            st.success(f"Found {len(competitors)} competitors")
        else:
            st.info(f"Found {len(existing_competitors)} existing competitors in the database.")
